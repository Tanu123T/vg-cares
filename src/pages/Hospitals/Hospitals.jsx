...
const HospitalsPage = () => {
  ...
  // 1. Snapshot the position of the active dropdown container
  const activeRef = openDropdown === 'country' ? countryRef : specialistRef;
  const initialRect = activeRef.current?.getBoundingClientRect();
  const initialTop = initialRect ? initialRect.top : 0;

  const handleScrollBehavior = (e) => {
    if (!activeRef.current) return;

    // 2. Ignore if scrolling inside the list itself
    if (e.target.classList?.contains('dropdown-floating-menu')) {
      return;
    }

    // 3. Detect if the main page has moved
    const currentRect = activeRef.current.getBoundingClientRect();
      ...
      <div className="filter-section">
        ...
        <div className="modern-dropdown" ref={countryRef}>
          ...
          {openDropdown === 'country' && (
            <div className="dropdown-floating-menu">
              {["all", "India", "USA", "UK", "UAE", "Germany","Singapore"].map(c => (
        ...
        <div className="modern-dropdown" ref={specialistRef}>
          ...
          {openDropdown === 'specialist' && (
            <div className="dropdown-floating-menu">
              {["all", "Cardiology", "Neurology", "Orthopedics", "Oncology", "Pediatrics","Gastroenterology"].map(s => (
